...
const Result = () => {
  ...
  const handlesearchresult = (e) => {
    ...
    axios
      ...
      .then(function (response) {
        console.log("API Response:", response.data.data); // Log the raw API response
        ...
        const filteredResults = response.data.data.filter(
          (result) => result.examination?.trim().toLowerCase() === formData.terms?.trim().toLowerCase()
        );
        ...
        if (filteredResults.length === 0) {
          toast.error("No results found for the selected exam term.");
        } else {
